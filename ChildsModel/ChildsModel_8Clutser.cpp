...
//
//  main.cpp
//  Codes_for_sharing_with_Sergio_and_Mercedes
//
...
// set<int> Union(int arr1[], int arr2[], int n1, int n2)
...
//  set<int> Intersection(int arr1[], int arr2[],int n1, int n2) {
...
//     int Sp1 = sizeof(SpacerSet1);
//     int Sp2 = sizeof(SpacerSet2);
...
int main(int argc, const char * argv[]) {
    ...
    int Replica = stoi(argv[6]);
    
    cout << "Rep = " << Replica << "\n"; 
    
//     std::mt19937 generator(1729);
    std::mt19937 generator(Replica);
    std::uniform_real_distribution<double> distribution(0.0,1.0);    
    
//     string stringReplica = argv[6];
    
    string stringReplica = to_string(Replica);
    
    // Parameters (can be compared with Childs' model. The names of those parametere should be self-explanatory):
    double CRISPRfailure = 1.0e-5;           // CRISPR failure rate even if the bacteria have the resistance.  //p in the model   ***
    double SpacerAcquisition = 1.0e-5;       // Spacer acquisition rate per bacterium phage encounter.          //q in the model  *****
    double carryingCapacity = 1e5*sqrt(10);   // Carrying capacity for the bacterial population.                // K in the model ***
    int burstSize = 50;                     // burst size for phages.                                           // \beta in the model 
    double adsorptionRate = 1.0e-7;          // adsorption rate.                                                // \phi in the model
    double decayRate = 0.1;                 // decay rate for phages.                                           \\ m in the model   ***
    double MutationRate = 5e-7;             // mutation rate for phages per phage generation per protospacer.   \\ \mu in the model  +++++
...
//     ofstream fileForBacteria("/Users/tongwang/Documents/Research/Simulations/C++ simulation/added_distributed_immunity_for_bacteria/temp data/TEMP/try/mu5e-7 S8P"+to_string(NumOfProtospacers)+" data bact"+" initialDiffDp"+to_string(Dp)+".txt");     // data1 records data of population from 1000h to 2500h per 1h
//     ofstream fileForPhage("/Users/tongwang/Documents/Research/Simulations/C++ simulation/added_distributed_immunity_for_bacteria/temp data/TEMP/try/mu5e-7 S8P"+to_string(NumOfProtospacers)+" data phage"+" initialDiffDp"+to_string(Dp)+".txt");
//     ofstream fileForTimeSeriesData("/Users/tongwang/Documents/Research/Simulations/C++ simulation/added_distributed_immunity_for_bacteria/temp data/TEMP/try/mu5e-7 S8P"+to_string(NumOfProtospacers)+" time series data P10.txt");

    // Write the file name: (You need to change the path according to your system.)
    //ofstream fileForBacteria("mu5e-7 S8P"+to_string(NumOfProtospacers)+" data bact"+" initialDiffDp"+to_string(Dp)+".txt");     // data1 records data of population from 1000h to 2500h per 1h
    //ofstream fileForPhage("mu5e-7 S8P"+to_string(NumOfProtospacers)+" data phage"+" initialDiffDp"+to_string(Dp)+".txt");
    //ofstream fileForTimeSeriesData("mu5e-7 S8P"+to_string(NumOfProtospacers)+" time series data P10.txt");
    
    
    ofstream fileForBacteria("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_data-bact"+".txt");     // data1 records data of population from 1000h to 2500h per 1h
    ofstream fileForPhage("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_data-phage"+".txt");
    ofstream fileForTimeSeriesData("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_time-series-data"+".txt");
    ofstream fileForBacteriaAbundance("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_Bacteria-abundance"+".txt");
    ofstream fileForBacteriaAnimation("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_Bacteria-animation"+".txt");
    ofstream fileForPhageAbundance("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_Phage-abundance"+".txt");
    ofstream fileForPhageAnimation("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_Phage-animation"+".txt");
    
    ofstream fileForPhageTree("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_Phage-TREE"+".txt"); //   intento de Arbol!!!!!
    ofstream fileForBacteriaTree("mu"+stringMutationRate+"_initialDiffDp"+stringDp+"_S"+to_string(NumOfSpacers)+"P"+to_string(NumOfProtospacers)+"_R-"+stringReplica+"_Bacteria-TREE"+".txt"); //   intento de Arbol!!!!!
